...
if __name__ == "__main__":
    for dataset_size in [10000]:
                ...
                for dataset_seed in dataset_seeds:
                    for alpha in alphas:
                        ...
                        if pack is not None:
                            ...
                            print(dataset_size, experiment_label, model_seed, dataset_seed, alpha)
        ...
        for experiment_label in experiment_labels:
            # Get mean/std of ~~hardest~~ easiest experiment
            mean_rel, std_rel = np.mean(
                rels_for_min_val[experiment_labels[0]]), np.std(rels_for_min_val[experiment_labels[-1]])
            for alpha in alphas:
                for dataset_seed in dataset_seeds:
                    for model_seed in model_seeds:
                        min_val_epoch = np.argmin(
                            val_losses[experiment_label][model_seed][dataset_seed][alpha])
                        close_rel = rel_losses[experiment_label][model_seed][dataset_seed][alpha] <= mean_rel + 2*std_rel
        ...
        for experiment_label in experiment_labels:
            ...
            cur_min_val = "{:.3f}".format(np.mean([list(nested_dict_values(d))
                                                for d in min_val[experiment_label].values()]))
            cur_epochs_to_min_val = "{:.2f}".format(np.mean([list(nested_dict_values(d))
                                                for d in epochs_to_min_val[experiment_label].values()]))
            cur_rel_for_min_val = "{:.2f}".format(np.mean([list(nested_dict_values(
                d)) for d in rel_for_min_val[experiment_label].values()]))
            cur_epochs_to_mean_rel = "{:.2f}".format(np.mean([list(nested_dict_values(
            ...
            cur_min_val_std = "{:.3f}".format(np.std([list(nested_dict_values(d))
                                                for d in min_val[experiment_label].values()]))
            cur_epochs_to_min_val_std = "{:.2f}".format(np.std([list(nested_dict_values(d))
                                                for d in epochs_to_min_val[experiment_label].values()]))
            cur_rel_for_min_val_std = "{:.2f}".format(np.std([list(nested_dict_values(
                d)) for d in rel_for_min_val[experiment_label].values()]))
            cur_epochs_to_mean_rel_std = "{:.2f}".format(np.std([list(nested_dict_values(
                d)) for d in epochs_to_mean_rel[experiment_label].values()]))
            ...
            print("{} & {} & ${} \pm {}$ & ${} \pm {}$ & ${} \pm {}$ & ${} \pm {}$ \\\\".format(
                ...
                cur_rel_for_min_val, cur_rel_for_min_val_std,
                cur_epochs_to_mean_rel, cur_epochs_to_mean_rel_std,
                cur_min_val, cur_min_val_std,
                cur_epochs_to_min_val, cur_epochs_to_min_val_std
            ))
        print("\hline")
